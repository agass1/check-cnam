...
This bot automatically checks the status of the health insurance policy (for citizens of the Republic of Moldova) and sends a Telegram notification if the policy has expired.

The check runs every 24 hours, with a timer implemented directly in the script for cross-platform operation without using cron or Task Scheduler.

For stable operation, it is recommended to add the script to your system’s autostart.
...
False — sends a notification only if status != ASIGURAT
